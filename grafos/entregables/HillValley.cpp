...
using namespace std;
int testNum = 1;
int bfs(int numRooms, int numDoors, int numSwitches, vector<pair<int, int>>& doorConnections, vector<pair<int, int>>& switchConnections) {
    ...
    while (!visitQueue.empty() && !finished) {
        int currentState = visitQueue.front();
        visitQueue.pop();
        for (int nextState : adjacencyList[currentState]) {
            if (!visited[nextState]) {
                visited[nextState] = true;
                bfsTree[nextState] = currentState;
                visitQueue.push(nextState);
            }
        }
    }
...
int main() {
    int numRooms, numDoors, numSwitches;
    while (cin >> numRooms >> numDoors >> numSwitches, numRooms != 0 || numDoors != 0 || numSwitches != 0) {
        ...
        bfs(numRooms, numDoors, numSwitches, doorConnections, switchConnections);
        testNum++;
    }
    return 0;
}
